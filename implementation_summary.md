...
## 6. Secure Logout and Token Revocation

To enhance security and provide a complete authentication lifecycle, a secure logout feature has been implemented, addressing SRS items `2.2.3` and `2.3.2`. This feature ensures that a user's token is immediately invalidated upon logout.

- **New Logout Endpoint:**
    - A new endpoint, `POST /api/v1/auth/logout`, has been added to the `auth-service`.
    - When a user calls this endpoint with their Bearer token, the token is permanently invalidated.

- **Revocation Mechanism:**
    1.  **JWT ID (`jti`):** All access tokens generated by the `auth-service` now include a unique token identifier (a `jti` claim).
    2.  **Redis Blacklist:** Upon logout, the `auth-service` extracts the token's `jti` and its remaining expiration time. It then stores this `jti` in a Redis cache, effectively adding it to a "blacklist" that persists until the token would have naturally expired.
    3.  **API Gateway Enforcement:** The authentication filter in the `api-gateway` has been upgraded. For every incoming request, it now performs a check against the Redis blacklist. If a token's `jti` is found in the blacklist, the gateway immediately rejects the request with a `401 Unauthorized` error, even if the token's signature and expiration date are still valid.

- **Security Benefit:** This implementation prevents the misuse of tokens after a user has explicitly logged out, closing a common security loophole in stateless authentication systems.
