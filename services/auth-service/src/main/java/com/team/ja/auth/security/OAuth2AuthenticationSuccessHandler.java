...
public class OAuth2AuthenticationSuccessHandler
     ...
     */
    @Value(
        "${app.oauth2.authorized-redirect-uris:http://localhost:3000/auth/callback}"
    )
     ...
     */
    @Value(
        "${app.oauth2.default-redirect-uri:http://localhost:3000/auth/callback}"
    )
    ...
    private AuthCredential upsertUser(ProviderUserInfo info) {
        Optional<AuthCredential> existingOpt =
            authCredentialRepository.findByEmail(info.getEmail());
    ...
    private ProviderUserInfo extractUserInfo(
        ...
        if (oauth2User instanceof OidcUser oidc) {
            ...
            if (!emailVerified) {
                emailVerified =
                    oidc.getEmailVerified() != null && oidc.getEmailVerified();
            }
        ...
        // Fall back to "name"
        if (
            !StringUtils.hasText(givenName) || !StringUtils.hasText(familyName)
        ) {
        ...
        // Safety defaults
        if (!StringUtils.hasText(givenName)) givenName = "";
        if (!StringUtils.hasText(familyName)) familyName = "";
